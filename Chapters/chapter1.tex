...
locomotion, such as controller-based techniques. Although effective by not requiring much space for the user to move, controller-based methods differ 
from natural walking in the real world, and have been shown less immersive. Therefore,
...
therefore, used in different contexts~\cite{Boletsis2019}.

One of the remaining challenges of \gls{VR} locomotion is the physical limitations users face when navigating a \gls{VE}.
...
provides higher feelings of presence by mirroring how humans walk~\cite{Nilsson2018}.
...
\glsfirst{RDW} refers to a group of techniques for locomotion in \gls{VR} that allow users to naturally walk through their \gls{VE}, by keeping  
users inside their available space through a variety of different redirection methods~\cite{8255772}. \gls{RDW} is most commonly associated with methods such as 
rotational gains, that play with the imperceptibility of human senses to slowly direct users to their available space.
\gls{RDW} techniques can also employ redirection with more overt strategies for 
safety-risking cases or others in which imperceptibility is kept by discreetly changing the \gls{VE} in real-time. 
Despite being a promising solution that enables walking in restricted spaces, 
...
Given the possibilities of shaping rules of reality in \gls{VR}, another form of implementing \gls{RDW} is by defying the real world Euclidean rules.
By employing non-Euclidean properties it is possible to create the illusion of a larger space than the actual limited tracking space, by using overlapping-architectures
...
space as if they were already positioned at the destination, with the appropriate transformations of position and rotation
relative to the portal~\cite{Lochner2021,Jaksties2022}.

However, portals as a means of locomotion have shown its limitations in previous studies~\cite{Ferreira2025,Rebelo2025}. 
...
as well as its naturalness, due to the unreal-like appearance of a floating door frame. 
...
implementation and evaluation of portal alternatives, aiming to counter-act the previously mentioned limitations.
...
The main objective of this dissertation is to design, implement and evaluate portals variants as a mean to redirect user walking 
in \gls{VR} within common small physical tracking spaces.
To complete this main objective, smaller goals have been identified for conducting this research:

\begin{itemize}

    \item \textbf{Review and test \gls{VR} locomotion techniques used in small physical spaces and their effects on \gls{UX}}:
    Review existing locomotion methods. Test traditional designs of portals to analyze usability issues reported by users.

    \item \textbf{Design and implement alternative portal designs}:
    Conceive and accomplish each of the designs, addressing each of the most common portal limitations found in previous studies.

    \item \textbf{Conduct user studies to evaluate the portal designs}: 
    Conduct empirical studies with participants in order to validate the portals in terms of spatial understanding, usability and user preference.
    
\end{itemize}

In order to better structure this research, the following three research questions are proposed,
based on remaining challenges of traditional portals, focusing on destination preview and user preference.:

\begin{itemize}

    \item \textbf{Q1 -- What are the effects of a dynamic preview in regard to users' sense of space?} 
    
    One of the identified limitations of using portals as a \gls{VR} locomotion technique in small physical spaces is 
    determining an appropriate portal position that provides an informative destination preview to support user orientation in the \gls{VE}. 
    This research question explores the impact on users' understanding of space when using alternative \gls{VR} portal techniques 
    that counteract this limitation by dynamically changing their previews.

     \item \textbf{Q2 -- What elements of \glsfirst{UX} do users prioritize when interacting with these navigation techniques?}
    ...
    \item \textbf{Q3 -- What are the navigation benefits of each portal design in small spaces?}

    This question seeks to identify the comparative advantages of each portal variant, highlighting the contexts in which 
    a given approach offers superior usability, efficiency, or user satisfaction.
...
Our solution relied on the implementation of three distinct portal variants that address the limitations of \gls{VR} 
navigation using portals (Figure~\ref{fig:solution})\footnote{Video Demonstration of the solution - \url{https://youtu.be/xcGcSZ7LP_c}, Last Access - Sep 2025 }.
...
where participants explored a museum \gls{VE} using each of the developed portal variants, bound by a physical space of 
$2.5\,\mathrm{m} \times 2.5\,\mathrm{m}$.
...
\begin{figure}[t]
    \centering
     \includegraphics[width=\textwidth]{NOVAthesisFiles/Images/screenshots/teaser image v2.png}
     \caption[The four portal techniques studied in this thesis.]
     {The four portal designs studied in this thesis: Traditional Portals, Movable Portals, Interactive Doors and Revolving Doors.}
     \label{fig:solution}
\end{figure}
...
This work contributed to further research the use of \glsfirst{RDW} techniques in small physical spaces, through the design, 
implementation and study of new variant portal designs that address identified limitations of common portal techniques used in 
\gls{VR} applications and research. The main contributions of this thesis are:

\begin{itemize}
    \item The design and implementation of three portal designs as a mean of \gls{RDW} in small physical spaces.;
    
    \item The study of the portal variants in comparison with portals commonly seen in \gls{VR} applications and research;
    
    \item Insights on the impact these portal variants have on \gls{UX};
    
    \item Insights on design guidelines for \gls{VR} locomotion;
\end{itemize}

The study done for this thesis has additionally resulted in the submission of a full paper to ICAT-EVGE 2025, detailing the developed portals 
and presenting and discussing the results of the study.
Furthermore, a full paper was accepted at ACM Multimedia 2025~\cite{RebeloACMM2025}, in which the student, as a co-author, 
contributed to conducting the user study. The results of this study were used to address the remaining challenges of traditional portals.
...
finishes by addressing locomotion techniques reliant on Non-Euclidean Spaces in \gls{VR}.

The \nameref{cha:analysis} chapter presents an in-depth analysis and the architecture of the system used to conduct this research.
This includes the design choices for the studied portal techniques.

The \nameref{cha:implementation} chapter addresses the implementation of all the components mentioned in the previous third chapter.
This includes the implementation of the portal techniques, the player entity used to bridge the real and virtual world, as well as 
\glsfirst{UI} elements of the experience.

The \nameref{cha:evaluation} chapter outlines the user study conducted for this work. Both protocol of the study and the results of 
the study are presented, and finally discussed, by addressing the research questions presented in the previous Section~\ref{sec:objectives-and-researh-questions}.

Finally, the \nameref{cha:conclusion} chapter presents the conclusions taken from this work, outlining the entirety of this work, as well as 
future work possibilities to explore and develop this research topic.