...
One method is through \textbf{Transitioning Areas}, which are transitional spaces that link different sections by recurring to the previously discussed 
...
Another method involves the use of \textbf{Portals}, which allow users to move seamlessly between different sections of the environment by presenting 
a preview of the connected space. This approach mitigates the disorientation that could arise from instantaneous repositioning, and will be discussed 
in more detail in Section~\ref{sec:portals}.
...
While prior work has demonstrated the potential of portals and self-overlapping architectures for extending navigable space, 
there remain exciting opportunities for further exploration. One area of interest is the placement of portals within a \gls{VE}. 
Portal placement can influence the preview of connected spaces~\cite{Ferreira2025, Rebelo2025}, as portals positioned near physical or virtual 
boundaries limit the visible area through the portal. Careful positioning is therefore important not only to maximize observable space but 
also to maintain the naturalness of the environment, as portals placed too far from walls can appear as floating door frames. 
Exploring strategies for optimal portal placement could enhance both the usability and spatial coherence of \glspl{VE}.
...
thus non-Euclidean geometry and its applications in \gls{VR} have been explored in this chapter as well.

The use of portals as a form of \gls{VR} locomotion was additionally explored. Portals offer an intuitive way of connecting separate areas of a virtual environment, 
allowing users to traverse large or discontinuous spaces whilst in a limited physical space.
The effectiveness of portals are, however, influenced by design choices. In particular, the placement of portals within a \gls{VE} plays a key 
role: positioning them near boundaries can restrict what users see through them, while placing them too far from walls can reduce spatial 
coherence by creating the impression of floating door frames.