...
and validated them in a user study. To support this development and study, a functional system was required. This chapter explores the design and concepts 
...
The chapter finalizes by presenting the designs of the portal variants studied in this work on Section~\ref{sec:portal-design}.

\section{Requirements}
\label{sec:requirements}


This research proposes the study of four portal designs as a way to redirect walking in large \gls{VE} examining its impact on 
navigation and \gls{UX}. It was essential to create a \gls{VR} experience within a \glsfirst{VE} that relied on the use of portals 
...
The system was designed to create a \gls{VE} that supports natural walking and hand-tracking to interact with virtual content. 
Hand-tracking was chosen over controller-based interaction to maximize feelings of presence and immersion. Users 
...
through the cameras present on the front of the \gls{HMD}. The hand-tracking is used for interacting with the \gls{VE}, whether it be for interacting with the 
portals, allowing users to navigate, or to interact with elements of the user study's task.
...
\gls{VR} which led to the submission and publication of a full paper for ACM Multimedia 2025~\cite{RebeloACMM2025}.
...
using one type of portal at a time. The study identified a key trade-off between the portal's distance from the wall and the clarity of the 
...
must be positioned at least 60–80 centimeters away from the virtual wall. However, this requirement introduces an unnatural “dead space” and 
...
\begin{itemize}
  ...
  \item Include variants of portal designs that address the limitations caused by portal placement and lack of preview.
  
  \item Include a common portal technique as a baseline for comparison with the variant designs.
  ...
  \item Task design should be simple enough to be repeated with different portal designs without introducing task-specific biases. 
...
space through the use of a self-overlapping virtual space. This setting also permits a more natural selection of exploratory and 
immersive tasks that rely on the use the developed portal variants. The chosen tasks were associated with exposition items and artworks 
...
The multiple rooms of the \gls{VE} should be connected through the portal variants, creating the self-overlapping architecture. In the interest 
...
. This design decision was made with consideration of how the different portal conditions could be evaluated in a user study. 
To avoid a lengthy and tiring experiment, we opted for a within-subjects design, allowing all participants to experience every 
portal condition within the same VE.

Throughout the user study users wear a \gls{HMD} and naturally walk around the physical environment of 2.5m x 2.5m. To interact the \gls{HMD} 
...
By employing a within-subject design, a uniform task structure, and consistent room layouts, the study ensures comparability across the 
portal designs and supports the collection of reliable data on usability, presence, and user experience.

\section{Analysis and Components}
...
This study aims to explore and study portal variants for \gls{VR} locomotion techniques based on natural walking. As such, we propose three 
...
This section presents the design choices underlying the four portal designs examined in our user study: Traditional Portals, serving as the baseline 
...
Thus, the designed interaction with this portal, represented in Figure~\ref{fig:mov-portal-interaction} would go as follows:
...
The interaction sequence, represented in Figure~\ref{fig:door-portal-interaction} can be summarized as follows:
...
The intended interaction sequence, represented in Figure~\ref{fig:rev-portal-interaction} is as follows:
...
Finally, the four portal variants investigated in this thesis were presented: Traditional Portals, Movable Portals, Interactive Doors, and Revolving Doors. Their intended interaction sequences were explained, along with the role of the Traditional Portals in serving as the baseline condition.