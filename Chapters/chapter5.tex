...
This chapter presents the design and results of this user study, exploring the findings and conclusions on 
the impact of the developed portal variants on usability, naturalness and spatial understanding.

This chapter begins with Section~\ref{sec:protocol}, which describes the protocol and experimental procedure of the user study 
adopted to evaluate the proposed portal designs. 
Section~\ref{sec:results} presents the results obtained from the study, followed by Section~\ref{sec:discussion}, 
which discusses these findings, addressing the research questions.

\section{User Study}

In order to evaluate the developed portal designs and answer the Research Questions proposed on Section~\ref{sec:objectives-and-researh-questions},
a user study was conducted. 
As mentioned in Section~\ref{sec:case-study}, a virtual museum tour is used as the case study, with multiple sections each with a 
self-overlapped architecture with their rooms connected by one of the implemented portal designs.
...
approximately 2.5m x 2.5m, providing conditions comparable to those available to most common \gls{VR} users~\cite{Azmandian2015, Liu2018b}. In addition, 
...
designs~—~Traditional Portals, Movable Portals, Interactive Doors, and Revolving Doors—while performing the same tasks within 
the same \glsfirst{VE}. The only variation between participants was the order in which the portal variants were presented, which 
followed a Latin Square design to minimize bias.

\subsection{Procedure}
\label{sec:procedure}

The experimental session commenced by asking the participant to read the informed consent presented in Appendix~\ref{app:consent}. After reading, 
agreeing and signing the consent form, users were asked to fill a pre-session Virtual Reality Sickness Questionnaire (VRSQ). 
...
from the others by theme, visual appearance, and the type of portal used to connect the rooms. Additionally, the starting room of each 
...
\textbf{1)~Exploration Task:} The user freely explored the rooms using the section's associated portal design, aiming to see and respond 
...
additional Likert-scale items assessing overall usability, a ranking of the four portal variants by preference, open-ended questions on perceived 
...
\textbf{Usability and Naturalness}: To assess subjective user experience, participants rated their agreement with two statements on a 7-point Likert scale (1=Strongly Disagree, 7=Strongly Agree): 

\begin{enumerate}
    \item “I found the transition between rooms natural”;
    \item “I found the transition between rooms intuitive / easy to use"
\end{enumerate}

To supplement subjective ratings, we captured an objective usability metric by logging usability hesitations. These were defined as any significant pause, verbal expression of confusion, or repeated, incorrect interaction attempts observed by the experimenter.
...
\textbf{Presence}: The overall sense of being "in" the virtual environment was measured using the Igroup Presence Questionnaire (IPQ).
...
The items related to the Virtual Reality Sickness Questionnaire (VRSQ) and Igroup Presence Questionnaire (IPQ) are presented in Appendix~\ref{app:Questionnaires}.
...
\begin{table}[h]
\centering
\caption{Population demographics tables.}
...
% Row 2: Visual Impairments | Education Level
\begin{subtable}[t]{0.45\textwidth}
\centering
\caption{Participants Visual Impairments (beyond the use of glasses).}
\begin{tabular}{l|c}
...
% Row 3: VR Experience | Video Game Experience
\begin{subtable}[t]{0.45\textwidth}
\centering
\caption{Participants VR Experience.}
\begin{tabular}{l|c}
VR Use & Participants(\#) \\
\cline{1-2}
Never used before & 8 \\
Used once or twice before  & 12 \\
Uses few times a year & 9 \\
Uses monthly & 1 \\
Uses weekly or more & 1 \\
\end{tabular}
\end{subtable}
\hfill
\begin{subtable}[t]{0.45\textwidth}
\centering
\caption{Participants Video Game Experience.}
\begin{tabular}{l|c}
Video Game Use & Participants(\#) \\
\cline{1-2}
Never & 2 \\
...
\begin{figure}[t]
\centering
\includegraphics[width=.75\linewidth]{NOVAthesisFiles/Images/graphs/naturalness_violin_plot.pdf}
...
other variants: Interactive Door (M=5.19, SD=1.74; p = .0013), Revolving Door (M=4.03, SD=1.56; p $<$ .001), and 
...
\begin{figure}[t]
\centering
\includegraphics[width=.75\linewidth]{NOVAthesisFiles/Images/graphs/ease_of_use_violin_plot.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{NOVAthesisFiles/Images/graphs/hesitations_stacked_horizontal_plot.pdf}
...
To assess overall preferences, participants ranked the four portal designs on a scale from 1 (most preferred) to 4 (least preferred) 
on the post-session questionnaire. An initial Friedman test, $\chi$²(3, N=30)= 70.67, p$<$0.001, shows a statistically significant 
difference in user preference between the four portal variants, indicating that participants did not rank all portals equally. 
...
Adjacent comparisons were conducted within each rank based on frequency, with only one reaching statistical significance: 
...
the second most top-ranked portal design.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{NOVAthesisFiles/Images/graphs/scenario_preference_plot.pdf}
    \caption[Participants preference rankings over portal variants.]
...
\begin{table}[h]
\centering
\caption{IPQ Subscale Results}
\begin{tabular}{l|c}
Subscale & Mean ± SD \\
\hline
Spatial Presence & 5.50 ± 0.45 \\
Involvement & 4.42 ± 0.40 \\
Experienced Realism & 4.58 ± 0.73 \\
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{VRSQ Results per Symptom.}
...
\caption{Pre-session}
\begin{tabular}{l|c|c}
Symptom & Mean ± SD & Min–Max \\
\hline
Discomfort & 0.35 ± 0.55 & 0-2 \\
Fatigue & 0.61 ± 0.67 & 0-2 \\
Eyestrain & 0.39 ± 0.50 & 0-1 \\
Difficulty Focusing & 0.32 ± 0.54 & 0-2 \\
Headache & 0.13 ± 0.34 & 0-1 \\
Fullness of Head & 0.23 ± 0.43 & 0-1 \\
Blurred Vision & 0.10 ± 0.30 & 0-1 \\
Dizzy & 0.10 ± 0.30 & 0-1 \\
Vertigo & 0.00 ± 0.00 & 0-0 \\
\end{tabular}
\end{subtable}
\hfill
% Post-session table
...
Symptom & Mean ± SD & Min–Max \\
\hline
Discomfort & 0.42 ± 0.67 & 0-2 \\
Fatigue & 0.39 ± 0.50 & 0-1 \\
Eyestrain & 0.58 ± 0.67 & 0-2 \\
Difficulty Focusing & 0.35 ± 0.49 & 0-1 \\
Headache & 0.16 ± 0.37 & 0-1 \\
Fullness of Head & 0.26 ± 0.51 & 0-2 \\
Blurred Vision & 0.29 ± 0.59 & 0-2 \\
Dizzy & 0.19 ± 0.48 & 0-2 \\
Vertigo & 0.03 ± 0.18 & 0-1 \\
\end{tabular}
\end{subtable}
...
and "most natural". Several participants noted that this portal design provided the closest experience to a real museum, as "in a real museum, 
...
(e.g., "This is fun!", "I can move it? How cool!"). Participants that preferred this portal over all
...
However, by the end of the session, participants reported greater comfort with the design (e.g., "After understanding it, it was easy.").
...
The main goal of the presented work was to evaluate four portal variants as a way to redirect walking within small physical spaces, 
...
architecture. In contrast, responses for the other portal variants were more widely dispersed, as answers no longer clustered on the same 
...
\noindent
\textbf{Q3 -- What are the navigation benefits of each portal design in small spaces?} 

Through the commentary and experience observed in this study, it was possible to conclude that each portal design 
...
with their guided redirection enhancing the perception of flow across rooms once participants became accustomed to the portal.

With these observations, the study achieved its goal of evaluating four portal variants as a means of enabling redirected walking 
in small physical spaces. 
By addressing the three research questions, it demonstrated that dynamic previews can enhance users' sense of spatial continuity, 
that ease of use and naturalness are the most valued aspects of interaction, and that each portal design offers unique navigation 
benefits suited to different contexts.

\section{Summary}
\label{sec:summary}

This chapter presented the evaluation of the developed portals variants through a user study with 31 
participants. 

The study protocol, procedure, and data collection methods were described, combining subjective questionnaires, objective performance measures, 
and qualitative feedback. Questionnaires included the Virtual Reality Sickness Questionnaire (VRSQ), 
the Igroup Presence Questionnaire (IPQ), and the Immersive Experience Questionnaire (IEQ), as well as Likert-scale 
items on ease of use and naturalness answered within the \gls{VE}. 
Objective measures focused on pointing angles in the spatial understanding task 
and observed usability hesitations, both spoken and action-based. Qualitative feedback was gathered through 
post-session open-ended questions, participant rankings of portal variants, and spontaneous comments or 
behaviors observed during the experience.

Results highlighted clear differences 
between portal designs in terms of spatial understanding, usability, naturalness, and user preference, as well as 
their influence on presence and cybersickness. This chapter was finalized with a discussion that connected these findings to the research 
questions, outlining the strengths and limitations of each design and their implications for \gls{VR} navigation 
design.
